...
# fig
using LinearAlgebra
function deg_bd2(hinge_x_offset, K_hsp)
    g = 9.81
...
function find_offset(K_hsp;hinge_x_offset_start = 0.019, target_deg = 100, max_iter = 50, tol = 1e-6,  h = 1e-4)
    hinge_x_offset = float(hinge_x_offset_start)

    
    for i in 1:max_iter
        f = deg_bd2(hinge_x_offset, K_hsp) - target_deg
        if norm(f) < tol
            return hinge_x_offset, "сошлось за $i шагов"
        end
        fp = (deg_bd2(hinge_x_offset + h, K_hsp) - deg_bd2(hinge_x_offset - h, K_hsp)) / (2h)
